    ...
    public class NativeFunctionCall : Object {
        ...
        Value Call<T>(List<Value> parametersOfSingleType) {
            ...
            if (paramCount == 2 || paramCount == 1) {
                ...
                throw new Exception("Unexpected number of parameters to NativeFunctionCall: " +
                                    parametersOfSingleType.Count);
            }
        }
        ...
        Value CallBinaryListOperation(List<Object> parameters) {
            ...
            throw new StoryException("Can not call use '" + name + "' operation on " + v1.valueType + " and " +
                                     v2.valueType);
        }
                ...
                foreach (Value val in parametersIn) {
                    if (val.valueType == ValueType.List) {
                        ...
                        if (list.TryGetItemWithValue(intVal, out item)) {
                            ...
                            throw new StoryException("Could not find List item with the value " + intVal + " in " +
                                                     list.name);
                    }
                    else
        ...
        static void GenerateNativeFunctionsIfNecessary() {
            if (_nativeFunctions == null) {
                ...
                BinaryOp<Path> divertTargetsEqual = (Path d1, Path d2) => { return d1.Equals(d2); };
                BinaryOp<Path> divertTargetsNotEqual = (Path d1, Path d2) => { return !d1.Equals(d2); };
