...
namespace Ink.Runtime {
    public static class Json {
        private static readonly string[] _controlCommandNames;
        ...
        static Json() {
            ...
            for (var i = 0; i < (int)ControlCommand.CommandType.TOTAL_VALUES; ++i)
        ...
        public static void WriteListRuntimeObjs(SimpleJson.Writer writer, List<Object> list) {
            ...
            foreach (var val in list) WriteRuntimeObject(writer, val);
        ...
        public static Dictionary<string, Object> JObjectToDictionaryRuntimeObjs(Dictionary<string, object> jObject) {
            ...
            foreach (var keyVal in jObject) dict[keyVal.Key] = JTokenToRuntimeObject(keyVal.Value);
        ...
        public static Dictionary<string, int> JObjectToIntDictionary(Dictionary<string, object> jObject) {
            ...
            foreach (var keyVal in jObject) dict[keyVal.Key] = (int)keyVal.Value;
            return dict;
        }
        ...
        public static Object JTokenToRuntimeObject(object token) {
            if (token is int || token is float || token is bool) return Value.Create(token);
            ...
            if (token is string) {
                ...
                for (var i = 0; i < _controlCommandNames.Length; ++i) {
                    ...
                    if (str == cmdName) return new ControlCommand((ControlCommand.CommandType)i);
                }
            ...
            if (token is Dictionary<string, object>) {
                ...
                // Tag
                if (obj.TryGetValue("#", out propValue)) return new Tag((string)propValue);
            ...
            if (token is List<object>) return JArrayToContainer((List<object>)token);
        ...
        public static void WriteRuntimeContainer(SimpleJson.Writer writer, Container container,
