    ...
    public class Container : Object, INamedContent {
        ...
        public Dictionary<string, Object> namedOnlyContent {
            get {
                ...
                foreach (var kvPair in namedContent) namedOnlyContentDict[kvPair.Key] = (Object)kvPair.Value;

                foreach (var c in content) {
                    var named = c as INamedContent;
                    if (named != null && named.hasValidName) namedOnlyContentDict.Remove(named.name);
                }
        ...
        public int countFlags {
            get {
                ...
                if (flags == CountFlags.CountStartOnly) flags = 0;
        ...
        public bool hasValidName => name != null && name.Length > 0;

        public void AddContent(Object contentObj) {
            content.Add(contentObj);

            if (contentObj.parent) throw new Exception("content is already in " + contentObj.parent);
        ...
        public void AddContent(IList<Object> contentList) {
            foreach (var c in contentList) AddContent(c);
        }
        ...
        public void TryAddNamedContent(Object contentObj) {
            ...
            if (namedContentObj != null && namedContentObj.hasValidName) AddToNamedContentOnly(namedContentObj);
        }
        ...
        protected Object ContentWithPathComponent(Path.Component component) {
            ...
            if (component.isParent) return parent;
        ...
        public void BuildStringOfHierarchy(StringBuilder sb, int indentation, Object pointedObj) {
            Action appendIndentation = () => {
                ...
                for (var i = 0; i < spacesPerIndent * indentation; ++i) sb.Append(" ");
            };

            appendIndentation();
            sb.Append("[");

            if (hasValidName) sb.AppendFormat(" ({0})", name);
            ...
            for (var i = 0; i < content.Count; ++i) {
                ...
                if (i != content.Count - 1) sb.Append(",");

                if (!(obj is Container) && obj == pointedObj) sb.Append("  <---");
