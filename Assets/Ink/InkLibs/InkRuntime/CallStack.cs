...
namespace Ink.Runtime {
    public class CallStack {
        ...
        public CallStack(CallStack toCopy) {
            ...
            foreach (var otherThread in toCopy._threads) _threads.Add(otherThread.Copy());
            _threadCounter = toCopy._threadCounter;
            _startOfRoot = toCopy._startOfRoot;
        }

        public List<Element> elements => callStack;

        public int depth => elements.Count;
        ...
        public int currentElementIndex => callStack.Count - 1;
        ...
        public bool canPop => callStack.Count > 1;

        public bool canPopThread => _threads.Count > 1 && !elementIsEvaluateFromGame;

        public bool elementIsEvaluateFromGame => currentElement.type == PushPopType.FunctionEvaluationFromGame;

        private List<Element> callStack => currentThread.callstack;

        public string callStackTrace {
            get {
                var sb = new StringBuilder();

                for (var t = 0; t < _threads.Count; t++) {
        ...
        public void SetJsonToken(Dictionary<string, object> jObject, Story storyContext) {
            ...
            foreach (var jThreadTok in jThreads) {
        ...
        public void WriteJson(SimpleJson.Writer w) {
            w.WriteObject(writer => {
                writer.WritePropertyStart("threads");
                {
                    ...
                    foreach (var thread in _threads) thread.WriteJson(writer);
        ...
        public class Thread {
            ...
            public Thread(Dictionary<string, object> jThreadObj, Story storyContext) : this() {
                ...
                var jThreadCallstack = (List<object>)jThreadObj["callstack"];
                foreach (var jElTok in jThreadCallstack) {
                    var jElementObj = (Dictionary<string, object>)jElTok;

                    var pushPopType = (PushPopType)(int)jElementObj["type"];
                    ...
                    if (jElementObj.TryGetValue("cPath", out currentContainerPathStrToken)) {
                        ...
                        if (threadPointerResult.approximate)
                            storyContext.Warning(
                                ...
                                currentContainerPathStr + "', so it was approximated to '" + pointer.container.path +
                                "' to recover. Has the story changed since this save data was created?");
                    }

                    var inExpressionEvaluation = (bool)jElementObj["exp"];
            ...
            public Thread Copy() {
                ...
                foreach (var e in callstack) copy.callstack.Add(e.Copy());
