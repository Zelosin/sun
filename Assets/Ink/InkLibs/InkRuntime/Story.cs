...
namespace Ink.Runtime {
    ...
    public class Story : Object {
        ...
        private readonly Dictionary<string, ExternalFunctionDef> _externals;
        private bool _hasValidatedExternals;

        private readonly Container _mainContentContainer;

        private readonly List<Container> _prevContainers = new();
        ...
        public Story(string jsonString) : this((Container)null) {
            ...
            var formatFromFile = (int)versionObj;
        ...
        public List<Choice> currentChoices {
            get {
                ...
                foreach (var c in state.currentChoices)
        ...
        /// </summary>
        public List<string> currentErrors => state.currentErrors;

        /// <summary>
        ///     Any warnings generated during evaluation of the Story.
        /// </summary>
        public List<string> currentWarnings => state.currentWarnings;
        ...
        /// </summary>
        public bool hasWarning => state.hasWarning;
        ...
        /// </summary>
        public VariablesState variablesState => state.variablesState;

        public ListDefinitionsOrigin listDefinitions { get; }
        ...
        public bool canContinue => state.canContinue;
        ...
        /// </summary>
        public bool asyncContinueComplete => !_asyncContinueActive;
        ...
        /// </summary>
        public List<string> globalTags => TagsAtStartOfFlowContainerWithPathString("");
        ...
        private DebugMetadata currentDebugMetadata {
            get {
                ...
                for (var i = state.callStack.elements.Count - 1; i >= 0; --i) {
                ...
                for (var i = state.outputStream.Count - 1; i >= 0; --i) {
        ...
        private void ToJson(SimpleJson.Writer writer) {
            ...
            if (listDefinitions != null) {
                ...
                foreach (var def in listDefinitions.lists) {
        ...
        private void ContinueInternal(float millisecsLimitAsync = 0) {
            ...
            do {
                ...
                if (_asyncContinueActive && durationStopwatch.ElapsedMilliseconds > millisecsLimitAsync) break;
            } while (canContinue);
            ...
            if (outputStreamEndsInNewline || !canContinue) {
                ...
                if (_stateSnapshotAtLastNewline != null) RestoreStateSnapshot();
                ...
                if (!canContinue) {
                    ...
                    if (state.generatedChoices.Count == 0 && !state.didSafeExit &&
                ...
                if (_recursiveContinueCount == 1)
                    state.variablesState.batchObservingVariableChanges = false;
        ...
        private bool ContinueSingleStep() {
            ...
            if (!canContinue && !state.callStack.elementIsEvaluateFromGame) TryFollowDefaultInvisibleChoice();
            ...
            if (!state.inStringEvaluation) {
                ...
                if (_stateSnapshotAtLastNewline != null) {
                    ...
                    if (change == OutputStateChange.NewlineRemoved) DiscardSnapshot();
                }
        ...
        private OutputStateChange CalculateNewlineOutputStateChange(string prevText, string currText, int prevTagCount,
            int currTagCount) {
            ...
            if (!newlineStillExists) return OutputStateChange.NewlineRemoved;
        ...
        public Pointer PointerAtPath(Path path) {
            ...
            if (result.obj == null || (result.obj == mainContentContainer && pathLengthToUse > 0))
                ...
                Warning("Failed to find content at path '" + path + "', so it was approximated to: '" +
                        result.obj.path + "'.");
        ...
        private void RestoreStateSnapshot() {
            ...
            if (!_asyncSaving) state.ApplyAnyPatch();
        }
        ...
        public void BackgroundSaveComplete() {
            ...
            if (_stateSnapshotAtLastNewline == null) state.ApplyAnyPatch();
        ...
        private void Step() {
            ...
            if (_profiler != null) _profiler.Step(state.callStack);
            ...
            if (choicePoint) {
                ...
                if (choice) state.generatedChoices.Add(choice);
        ...
        private void VisitChangedContainersDueToDivert() {
            ...
            while (currentContainerAncestor && (!_prevContainers.Contains(currentContainerAncestor) ||
                ...
                var enteringAtStart = currentContainerAncestor.content.Count > 0
        ...
        private bool IsTruthy(Object obj) {
            ...
            if (obj is Value) {
                ...
                if (val is DivertTargetValue) {
                    ...
                    Error("Shouldn't use a divert target (to " + divTarget.targetPath +
        ...
        private bool PerformLogicAndFlowControl(Object contentObj) {
            ...
            if (contentObj is Divert) {
                var currentDivert = (Divert)contentObj;
                ...
                if (currentDivert.hasVariableTarget) {
                    ...
                    if (varContents == null) {
                        ...
                        var errorMessage = "Tried to divert to a target from a variable, but the variable (" + varName +
            ...
            if (contentObj is ControlCommand) {
                ...
                switch (evalCommand.commandType) {
                    ...
                    case ControlCommand.CommandType.PopTunnel:
                        ...
                        if (state.TryExitFunctionEvaluationFromGame()) break;
                        ...
                        if (state.callStack.currentElement.type != popType || !state.callStack.canPop) {
                            ...
                            if (!state.callStack.canPop) expected = "end of flow (-> END or choice)";
                    ...
                    case ControlCommand.CommandType.BeginString:
                        ...
                        Assert(state.inExpressionEvaluation,
                    ...
                    case ControlCommand.CommandType.EndString:
                        ...
                        var sb = new StringBuilder();
                        foreach (var c in contentStackForString) sb.Append(c.ToString());
                    ...
                    case ControlCommand.CommandType.Random: {
                        ...
                        if (randomRange <= 0)
                            Error("RANDOM was called with minimum as " + minInt.value + " and maximum as " +
                                  maxInt.value + ". The maximum must be larger");
                    ...
                    case ControlCommand.CommandType.ListRandom: {
                        ...
                        // Non-empty source list
                            ...
                            for (var i = 0; i <= listItemIndex; i++) listEnumerator.MoveNext();
            ...
            if (contentObj is VariableReference) {
                ...
                // Normal variable reference
                    ...
                    if (foundValue == null) {
                        Warning("Variable not found: '" + varRef.name +
        ...
        public void ChoosePathString(string path, bool resetCallstack = true, params object[] arguments) {
            ...
            if (resetCallstack) {
                ...
                if (state.callStack.currentElement.type == PushPopType.Function) {
                    ...
                    throw new Exception("Story was running a function " + funcDetail +
        ...
        ///     The arguments that the ink function takes, if any. Note that we don't (can't) do any validation
        ///     on the number of arguments right now, so make sure you get it right!
        /// </param>
        ...
        ///     This text output is any text written as normal content within the function, as opposed to the return value, as
        ...
        public object EvaluateFunction(string functionName, out string textOutput, params object[] arguments) {
            ...
            while (canContinue) stringOutput.Append(Continue());
        ...
        public Object EvaluateExpression(Container exprContainer) {
            ...
            if (state.callStack.elements.Count > startCallStackHeight) state.PopCallstack();
        ...
        public void CallExternalFunction(string funcName, int numberOfArguments) {
            ...
            // Run the function!
            var funcResult = funcDef.function(arguments.ToArray());
        ...
        private object TryCoerce<T>(object value) {
            ...
            if (value is float && typeof(T) == typeof(int)) {
                var intVal = (int)Math.Round((float)value);
                return intVal;
            }
        ...
        private void ValidateExternalBindings(Object o, HashSet<string> missingExternals) {
            ...
            if (divert && divert.isExternal) {
                ...
                if (!_externals.ContainsKey(name)) {
                    if (allowExternalFunctionFallbacks) {
                        var fallbackFound = mainContentContainer.namedContent.ContainsKey(name);
        ...
        public void ObserveVariable(string variableName, VariableObserver observer) {
            ...
            if (!state.variablesState.GlobalVariableExistsWithName(variableName))
                throw new Exception("Cannot observe variable '" + variableName +
        ...
        public void ObserveVariables(IList<string> variableNames, VariableObserver observer) {
            foreach (var varName in variableNames) ObserveVariable(varName, observer);
        }
        ...
        public void RemoveVariableObserver(VariableObserver observer = null, string specificVariableName = null) {
            ...
            // Remove observer for all variables
                ...
                foreach (var varName in keys) {
                    ...
                    if (_variableObservers[varName] == null) _variableObservers.Remove(varName);
                }
            }
        }
        ...
        private void NextContent() {
            ...
            if (!successfulPointerIncrement) {
                ...
                if (state.callStack.CanPop(PushPopType.Function)) {
                    ...
                    if (state.inExpressionEvaluation) state.PushEvaluationStack(new Void());
                ...
                if (didPop && !state.currentPointer.isNull) NextContent();
            }
        }
        ...
        private bool IncrementContentPointer() {
            ...
            while (pointer.index >= pointer.container.content.Count) {
                ...
                var nextAncestor = pointer.container.parent as Container;
        ...
        private int NextSequenceShuffleIndex() {
            ...
            var sequenceHash = 0;
            foreach (var c in seqPathStr) sequenceHash += c;
            var randomSeed = sequenceHash + loopIndex + state.storySeed;
            var random = new Random(randomSeed);

            var unpickedIndices = new List<int>();
            for (var i = 0; i < numElements; ++i) unpickedIndices.Add(i);
        ...
        private void AddError(string message, bool isWarning = false, bool useEndLineNumber = false) {
            ...
            if (dm != null) {
                ...
                message = string.Format("RUNTIME {0}: '{1}' line {2}: {3}", errorTypeStr, dm.fileName, lineNum,
                    message);
            }
        ...
        private void Assert(bool condition, string message = null, params object[] formatParams) {
            if (condition == false) {
                if (message == null) message = "Story assert";
                if (formatParams != null && formatParams.Count() > 0) message = string.Format(message, formatParams);
