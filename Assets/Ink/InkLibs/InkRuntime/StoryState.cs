...
namespace Ink.Runtime {
    ...
    public class StoryState {
        ...
        public StoryState(Story story) {
            ...
            var timeSeed = DateTime.Now.Millisecond;
        ...
        public int callstackDepth => callStack.depth;

        // REMEMBER! REMEMBER! REMEMBER!
        // When adding state, update the Copy method, and serialisation.
        // REMEMBER! REMEMBER! REMEMBER!

        public List<Object> outputStream => _currentFlow.outputStream;
        ...
        public List<Choice> generatedChoices => _currentFlow.currentChoices;
        ...
        public CallStack callStack => _currentFlow.callStack;
        ...
        public Pointer currentPointer {
            ...
            set => callStack.currentElement.currentPointer = value;
        }

        public Pointer previousPointer {
            get => callStack.currentThread.previousPointer;
            set => callStack.currentThread.previousPointer = value;
        }

        public bool canContinue => !currentPointer.isNull && !hasError;

        public bool hasError => currentErrors != null && currentErrors.Count > 0;

        public bool hasWarning => currentWarnings != null && currentWarnings.Count > 0;
            ...
            get {
                if (_outputStreamTextDirty) {
                    ...
                    foreach (var outputObj in outputStream) {
                        ...
                        if (textContent != null) sb.Append(textContent.value);
                    }
            ...
            get {
                if (_outputStreamTagsDirty) {
                    ...
                    foreach (var outputObj in outputStream) {
                        ...
                        if (tag != null) _currentTags.Add(tag.text);
                    }
        ...
        public bool inStringEvaluation {
            get {
                for (var i = outputStream.Count - 1; i >= 0; i--) {
                    ...
                    if (cmd && cmd.commandType == ControlCommand.CommandType.BeginString) return true;
                }
        ...
        public int VisitCountForContainer(Container container) {
            if (!container.visitsShouldBeCounted) {
                story.Error("Read count for target (" + container.name + " - on " + container.debugMetadata +
                            ") unknown.");
                return 0;
            }
        ...
        public int TurnsSinceForContainer(Container container) {
            if (!container.turnIndexShouldBeCounted)
                story.Error("TURNS_SINCE() for target (" + container.name + " - on " + container.debugMetadata +
                            ") unknown.");

            var index = 0;

            if (_patch != null && _patch.TryGetTurnIndex(container, out index)) return currentTurnIndex - index;
        ...
        internal void RemoveFlow_Internal(string flowName) {
            ...
            if (_currentFlow.name == flowName) SwitchToDefaultFlow_Internal();
        ...
        private void LoadJsonObj(Dictionary<string, object> jObject) {
            ...
            // Old format: individually load up callstack, output stream, choices in current/default flow
                ...
                _currentFlow.currentChoices =
                    Json.JArrayToRuntimeObjList<Choice>((List<object>)jObject["currentChoices"]);
        ...
        public void PushToOutputStream(Object obj) {
            ...
            if (text) {
                ...
                if (listText != null) {
                    foreach (var textObj in listText) PushToOutputStreamIndividual(textObj);
                    OutputStreamDirty();
                    return;
                }
            }
        ...
        private void PushToOutputStreamIndividual(Object obj) {
            ...
            // We also need to know when to stop trimming, when there's non-whitespace.
                ...
                if (currEl.type == PushPopType.Function) functionTrimIndex = currEl.functionStartInOuputStream;
        ...
        private void TrimNewlinesFromOutputStream() {
            ...
            while (i >= 0) {
                ...
                if (cmd || (txt && txt.isNonWhitespace))
                    break;
                if (txt && txt.isNewline) removeWhitespaceFrom = i;
                i--;
            }
        ...
        public List<Object> PopEvaluationStack(int numberOfObjects) {
            if (numberOfObjects > evaluationStack.Count) throw new Exception("trying to pop too many objects");
        ...
        private void TrimWhitespaceFromFunctionEnd() {
            ...
            for (var i = outputStream.Count - 1; i >= functionStartPoint; i--) {
                var obj = outputStream[i];
                var txt = obj as StringValue;
                var cmd = obj as ControlCommand;
                if (!txt) continue;
                ...
                for (var i = 0; i < arguments.Length; i++) {
                    if (!(arguments[i] is int || arguments[i] is float || arguments[i] is string ||
                        ...
                        throw new ArgumentException(
                            "ink arguments when calling EvaluateFunction / ChoosePathStringWithParameters must be int, float, string or InkList. Argument was " +
        ...
        public object CompleteFunctionEvaluationFromGame() {
            ...
            if (returnedObj) {
                ...
                if (returnVal.valueType == ValueType.DivertTarget) return returnVal.valueObject.ToString();
