...
namespace Ink.Runtime {
	...
	public class Profiler {
        private static readonly double _millisecsPerTick = 1000.0 / Stopwatch.Frequency;

        private double _continueTotal;

        private readonly Stopwatch _continueWatch = new();
        ...
        private readonly List<StepDetails> _stepDetails = new();
        ...
        /// </summary>
        public ProfileNode rootNode { get; }
        ...
        public void PostStep() {
            ...
            rootNode.AddSample(_currStepStack, duration);
        ...
        public string StepLengthReport() {
            ...
            sb.AppendLine("TOTAL: " + rootNode.totalMillisecs + "ms");

            var averageStepTimes = _stepDetails
                .GroupBy(s => s.type)
                .Select(typeToDetails =>
                    new KeyValuePair<string, double>(typeToDetails.Key, typeToDetails.Average(d => d.time)))
            ...
            var accumStepTimes = _stepDetails
                .GroupBy(s => s.type)
                .Select(typeToDetails =>
                    new KeyValuePair<string, double>(typeToDetails.Key + " (x" + typeToDetails.Count() + ")",
                        typeToDetails.Sum(d => d.time)))
        ...
        public static string FormatMillisecs(double num) {
            if (num > 5000) return string.Format("{0:N1} secs", num / 1000.0);
            if (num > 1000)
...
#pragma warning restore 0649
        ...
        /// </summary>
        public int totalMillisecs => (int)_totalMillisecs;
        ...
        private void PrintHierarchy(StringBuilder sb, int indent) {
            ...
            foreach (var keyNode in descendingOrderedNodes) keyNode.Value.PrintHierarchy(sb, indent + 1);
        }
