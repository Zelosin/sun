...
namespace Ink.Runtime {
    ...
    public class VariablesState : IEnumerable<string> {
        ...
        private readonly ListDefinitionsOrigin _listDefsOrigin;
        ...
        public bool batchObservingVariableChanges {
            get => _batchObservingVariableChanges;
            set {
        ...
        public object this[string variableName] {
            ...
            set {
                if (!_defaultGlobalVariables.ContainsKey(variableName))
                    throw new StoryException("Cannot assign to a variable (" + variableName +
                ...
                if (val == null) {
                    ...
                    throw new Exception("Invalid value passed to VariableState: " + value);
                }
        ...
        public void ApplyPatch() {
            foreach (var namedVar in patch.globals) _globalVariables[namedVar.Key] = namedVar.Value;
        ...
        public bool RuntimeObjectsEqual(Object obj1, Object obj2) {
            ...
            if (boolVal != null) return boolVal.value == ((BoolValue)obj2).value;

            var intVal = obj1 as IntValue;
            if (intVal != null) return intVal.value == ((IntValue)obj2).value;

            var floatVal = obj1 as FloatValue;
            if (floatVal != null) return floatVal.value == ((FloatValue)obj2).value;

            // Other Value type (using proper Equals: list, string, divert path)
            var val1 = obj1 as Value;
            var val2 = obj2 as Value;
            if (val1 != null) return val1.valueObject.Equals(val2.valueObject);

            throw new Exception("FastRoughDefinitelyEquals: Unsupported runtime object type: " + obj1.GetType());
        }
        ...
        public bool GlobalVariableExistsWithName(string name) {
            return _globalVariables.ContainsKey(name) ||
                   (_defaultGlobalVariables != null && _defaultGlobalVariables.ContainsKey(name));
        }
        ...
        private Object GetVariableWithName(string name, int contextIndex) {
            ...
            if (varPointer) varValue = ValueAtVariablePointer(varPointer);
