...
namespace Ink {
    public partial class InkParser {
        ...
        protected Conditional InnerConditionalContent(Expression initialQueryExpression) {
            ...
            var isInline = Parse(Newline) == null;

            if (isInline && !canBeInline) return null;
            ...
            // Multiline innards
                ...
                if (alternatives == null) {
                    ...
                    if (initialQueryExpression) {
                        var soleContent = StatementsAtLevel(StatementLevel.InnerBlock);
                        if (soleContent != null) {
                            ...
                            if (elseBranch) {
                                if (!elseBranch.isElse) {
                                    ErrorWithParsedObject(
                                        "Expected an '- else:' clause here rather than an extra condition", elseBranch);
                ...
                if (initialQueryExpression) {
                    ...
                    for (var i = 0; i < alternatives.Count; ++i) {
                        ...
                        var isLast = i == alternatives.Count - 1;
                        ...
                        // }
                            ...
                            if (!isLast && alternatives.Count > 2) {
                                ErrorWithParsedObject("Only final branch can be an 'else'. Did you miss a ':'?",
                                    branch);
                            }
                            else {
                                if (i == 0)
                            ...
                            if (isLast) {
                                ...
                                if (alt.isElse) {
                                    ...
                                    if (finalClause.isElse)
                                        ...
                                        ErrorWithParsedObject(
                                            "'else' case in conditional should always be the final one", alt);
                                }
                                else {
                                    ErrorWithParsedObject("Branch doesn't have condition. Are you missing a ':'? ",
                                        alt);
                                }
                            }
                        }
            ...
            foreach (var branch in alternatives) branch.isInline = isInline;
        ...
        protected List<ConditionalSingleBranch> InlineConditionalBranches() {
            ...
            if (listOfLists == null || listOfLists.Count == 0) return null;
        ...
        protected List<ConditionalSingleBranch> MultilineConditionalBranches() {
            ...
            var multipleConditions = OneOrMore(SingleMultilineCondition);
        ...
        protected ConditionalSingleBranch SingleMultilineCondition() {
            ...
            var isElse = Parse(ElseExpression) != null;
