...
namespace Ink {
    public partial class InkParser {
        ...
        protected Object TempDeclarationOrAssignment() {
            ...
            var isNewDeclaration = ParseTempKeyword();
            ...
            if (varIdentifier == null) return null;

            Whitespace();

            // += -=
            var isIncrement = ParseString("+") != null;
            var isDecrement = ParseString("-") != null;
            ...
            var assignedExpression = (Expression)Expect(Expression, "value expression to be assigned");
        ...
        protected Return ReturnStatement() {
            ...
            if (returnOrDone != "return") return null;
        ...
        protected Expression Expression(int minimumPrecedence) {
            ...
            while (true) {
                ...
                if (infixOp != null && infixOp.precedence > minimumPrecedence) {
                    ...
                    var multiaryExpr = Expect(() => ExpressionInfixRight(expr, infixOp), expectationMessage);
        ...
        protected Expression ExpressionUnary() {
            ...
            if (divertTarget != null) return divertTarget;
            ...
            if (prefixOp == null) prefixOp = Parse(ExpressionNot);
            ...
            var expr = OneOf(ExpressionList, ExpressionParen, ExpressionFunctionCall, ExpressionVariableName,
                ExpressionLiteral) as Expression;

            // Only recurse immediately if we have one of the (usually optional) unary ops
            if (expr == null && prefixOp != null) expr = ExpressionUnary();

            if (expr == null)
                return null;

            if (prefixOp != null) expr = UnaryExpression.WithInner(expr, prefixOp);
        ...
        protected StringExpression ExpressionString() {
            ...
            var textAndLogic = Parse(MixedTextAndLogic);
        ...
        protected Number ExpressionBool() {
            ...
            if (id == "false") return new Number(false);
        ...
        protected Expression ExpressionVariableName() {
            var path = Interleave<Identifier>(IdentifierWithMetadata, Exclude(Spaced(String("."))));
        ...
        protected List ExpressionList() {
            ...
            var memberNames = SeparatedList(ListMember, Spaced(String(",")));
        ...
        protected Identifier ListMember() {
            ...
            if (dot != null) {
                var identifier2 =
                    Expect(IdentifierWithMetadata, "element name within the set " + identifier) as Identifier;
        ...
        private void RegisterExpressionOperators() {
            ...
            RegisterBinaryOperator("?", 3);
            RegisterBinaryOperator("has", 3, true);
            RegisterBinaryOperator("!?", 3);
            RegisterBinaryOperator("hasnt", 3, true);
            RegisterBinaryOperator("^", 3);

            RegisterBinaryOperator("+", 4);
            RegisterBinaryOperator("-", 5);
            RegisterBinaryOperator("*", 6);
            RegisterBinaryOperator("/", 7);

            RegisterBinaryOperator("%", 8);
