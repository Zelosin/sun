    ...
    public partial class InkParser {
        ...
        protected List<Object> StatementsAtLevel(StatementLevel level) {
            ...
            if (level == StatementLevel.InnerBlock) {
                var badGatherDashCount = Parse(GatherDashes);
        ...
        protected object StatementAtLevel(StatementLevel level) {
            var rulesAtLevel = _statementRulesAtLevel[(int)level];
        ...
        protected object StatementsBreakForLevel(StatementLevel level) {
            ...
            var breakRules = _statementBreakRulesAtLevel[(int)level];
        ...
        private void GenerateStatementLevelRules() {
            ...
            foreach (var level in levels) {
                var rulesAtLevel = new List<ParseRule>();
                ...
                if (level > StatementLevel.InnerBlock) rulesAtLevel.Add(Gather);

                // Stitches (and gathers) can (currently) only go in Knots and top level
                if (level >= StatementLevel.Knot) rulesAtLevel.Add(StitchDefinition);
                ...
                if (level <= StatementLevel.Knot) breakingRules.Add(KnotDeclaration);

                // Break current stitch with a new stitch
                if (level <= StatementLevel.Stitch) breakingRules.Add(StitchDeclaration);
