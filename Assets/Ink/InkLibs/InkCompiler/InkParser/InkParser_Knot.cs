...
namespace Ink {
    public partial class InkParser {
        protected Knot KnotDefinition() {
            ...
            Expect(EndOfLine, "end of line after knot name definition", SkipToNextLine);

            ParseRule innerKnotStatements = () => StatementsAtLevel(StatementLevel.Knot);

            var content =
                Expect(innerKnotStatements, "at least one line within the knot",
                    KnotStitchNoContentRecoveryRule) as List<Object>;
        ...
        protected FlowDecl KnotDeclaration() {
            ...
            var isFunc = identifier?.name == "function";
            if (isFunc) {
            ...
            var parameterNames = Parse(BracketedKnotDeclArguments);
        ...
        protected object StitchDefinition() {
            ...
            Expect(EndOfLine, "end of line after stitch name", SkipToNextLine);

            ParseRule innerStitchStatements = () => StatementsAtLevel(StatementLevel.Stitch);

            var content = Expect(innerStitchStatements, "at least one line within the stitch",
                KnotStitchNoContentRecoveryRule) as List<Object>;
        ...
        protected FlowDecl StitchDeclaration() {
            ...
            var isFunc = ParseString("function") != null;
            if (isFunc) Whitespace();

            var stitchName = Parse(IdentifierWithMetadata);
            if (stitchName == null)
                return null;

            Whitespace();

            var flowArgs = Parse(BracketedKnotDeclArguments);
        ...
        protected List<FlowBase.Argument> BracketedKnotDeclArguments() {
            ...
            if (flowArguments == null) flowArguments = new List<FlowBase.Argument>();
        ...
        protected FlowBase.Argument FlowDeclArgument() {
            ...
            if (divertArrow != null) flowArg.isDivertTarget = true;

            // Passing by reference
            if (firstIden != null && firstIden.name == "ref") {
                if (secondIden == null) Error("Expected an parameter name after 'ref'");
            ...
            // Simple argument name
                ...
                if (flowArg.identifier == null) Error("Expected an parameter name");
        ...
        protected ExternalDeclaration ExternalDeclaration() {
            ...
            var external = Parse(IdentifierWithMetadata);
            if (external == null || external.name != "EXTERNAL")
                return null;

            Whitespace();

            var funcIdentifier = Expect(IdentifierWithMetadata, "name of external function") as Identifier ??
                                 new Identifier();

            Whitespace();

            var parameterNames =
                Expect(BracketedKnotDeclArguments,
                        "declaration of arguments for EXTERNAL, even if empty, i.e. 'EXTERNAL " + funcIdentifier +
                        "()'") as
                    List<FlowBase.Argument>;
