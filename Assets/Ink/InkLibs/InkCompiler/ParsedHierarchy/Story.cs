...
namespace Ink.Parsed {
    public class Story : FlowBase {
        ...
        private readonly HashSet<Container> _dontFlattenContainers = new();
        ...
        public Story(List<Object> toplevelObjects, bool isInclude = false) : base(null, toplevelObjects,
            isIncludedStory: isInclude) {
            // Don't do anything much on construction, leave it lightweight until
            // the ExportRuntime method is called.
        }

        public override FlowLevel flowLevel => FlowLevel.Story;
        ...
        internal bool hadWarning { get; private set; }
        ...
        public Runtime.Story ExportRuntime(ErrorHandler errorHandler = null) {
                ...
                if (constants.TryGetValue(constDecl.constantName, out existingDefinition))
                    if (!existingDefinition.Equals(constDecl.expression)) {
                        var errorMsg = string.Format(
                            "CONST '{0}' has been redefined with a different value. Multiple definitions of the same CONST are valid so long as they contain the same value. Initial definition was on {1}.",
                            constDecl.constantName, existingDefinition.debugMetadata);
            ...
            foreach (var listDef in FindAll<ListDefinition>()) _listDefs[listDef.identifier?.name] = listDef;
                ...
                if (itemInThisList) {
                    if (foundItem != null) {
                        Error(
                            ...
                            originalFoundList.identifier + " and " + listToSearch.identifier, source, false);
                    }
                    else {
        ...
        private void TryFlattenContainer(Container container) {
            if (container.namedContent.Count > 0 || container.hasValidName ||
                _dontFlattenContainers.Contains(container))
                return;
        ...
        public override void Error(string message, Object source, bool isWarning) {
            var errorType = isWarning ? ErrorType.Warning : ErrorType.Error;
            ...
            if (source && source.debugMetadata != null && source.debugMetadata.startLineNumber >= 1) {
                if (source.debugMetadata.fileName != null) sb.AppendFormat("'{0}' ", source.debugMetadata.fileName);
        ...
        private void NameConflictError(Object obj, string name, Object existingObj, string typeNameToPrint) {
            obj.Error(typeNameToPrint + " '" + name + "': name has already been used for a " +
                      existingObj.typeName.ToLower() + " on " + existingObj.debugMetadata);
        }
        ...
        public void CheckForNamingCollisions(Object obj, Identifier identifier, SymbolType symbolType,
            string typeNameOverride = null) {
            var typeNameToPrint = typeNameOverride ?? obj.typeName;
            if (IsReservedKeyword(identifier?.name)) {
                obj.Error("'" + name + "' cannot be used for the name of a " + typeNameToPrint.ToLower() +
            ...
            // Top level knots
            var knotOrFunction = ContentWithNameAtLevel(identifier?.name, FlowLevel.Knot) as FlowBase;
                ...
                if (flow && flow.hasParameters)
                    foreach (var arg in flow.arguments)
                        if (arg.identifier?.name == identifier?.name) {
                            obj.Error(typeNameToPrint + " '" + name +
                                      "': Name has already been used for a argument to " + flow.identifier + " on " +
                                      flow.debugMetadata);
                            return;
                        }
            }
