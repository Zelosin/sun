...
namespace Ink.Parsed {
    ...
    public abstract class FlowBase : Object, INamedContent {
        ...
        public FlowBase(Identifier name = null, List<Object> topLevelObjects = null, List<Argument> arguments = null,
            bool isFunction = false, bool isIncludedStory = false) {
            identifier = name;

            if (topLevelObjects == null) topLevelObjects = new List<Object>();

            // Used by story to add includes
            PreProcessTopLevelObjects(topLevelObjects);

            topLevelObjects = SplitWeaveAndSubFlowContent(topLevelObjects, this is Story && !isIncludedStory);
        ...
        public bool hasParameters => arguments != null && arguments.Count > 0;

        public abstract FlowLevel flowLevel { get; }
        public bool isFunction { get; protected set; }

        protected Dictionary<string, FlowBase> subFlowsByName { get; private set; }

        public override string typeName {
            get {
                if (isFunction) return "Function";
                return flowLevel.ToString();
            }
        }
        ...
        private List<Object> SplitWeaveAndSubFlowContent(List<Object> contentObjs, bool isRootStory) {
            ...
            subFlowsByName = new Dictionary<string, FlowBase>();
            ...
            foreach (var obj in contentObjs) {
                ...
                if (subFlow) {
                    ...
                    subFlowObjs.Add(obj);
                    subFlowsByName[subFlow.identifier?.name] = subFlow;
                }
                else {
                    weaveObjs.Add(obj);
                }
            ...
            if (subFlowObjs.Count > 0) finalContent.AddRange(subFlowObjs);
        ...
        public void TryAddNewVariableDeclaration(VariableAssignment varDecl) {
            ...
            if (variableDeclarations.ContainsKey(varName)) {
                ...
                Error("found declaration variable '" + varName + "' that was already declared" + prevDeclError,
                    varDecl);
        ...
        public void ResolveWeavePointNaming() {
            ...
            if (subFlowsByName != null)
                foreach (var namedSubFlow in subFlowsByName)
            ...
            // Non-functon: Make sure knots and stitches don't attempt to use Return statement
                ...
                if (foundReturn != null)
                    Error(
                        ...
                        identifier + " ==", foundReturn);
            }

            var container = new Container();
            container.name = identifier?.name;

            if (story.countAllVisits) container.visitsShouldBeCounted = true;
        ...
        private void GenerateArgumentVariableAssignments(Container container) {
            ...
            for (var i = arguments.Count - 1; i >= 0; --i) {
                ...
                var assign = new Runtime.VariableAssignment(paramName, true);
        ...
        public Object ContentWithNameAtLevel(string name, FlowLevel? level = null, bool deepSearch = false) {
            ...
            if (subFlowsByName.TryGetValue(name, out subFlow))
        ...
        private Object DeepSearchForAnyLevelContent(string name) {
            var weaveResultSelf = ContentWithNameAtLevel(name, FlowLevel.WeavePoint, false);
            if (weaveResultSelf) return weaveResultSelf;

            foreach (var subFlowNamePair in subFlowsByName) {
        ...
        public override void ResolveReferences(Story context) {
            if (_startingSubFlowDivert) _startingSubFlowDivert.targetPath = _startingSubFlowRuntime.path;
        ...
        private void CheckForDisallowedFunctionFlowControl() {
            ...
            foreach (var subFlowAndName in subFlowsByName) {
                var name = subFlowAndName.Key;
                var subFlow = subFlowAndName.Value;
                Error(
                    "Functions may not contain stitches, but saw '" + name + "' within the function '" + identifier +
                    "'", subFlow);
            }
            ...
            foreach (var divert in allDiverts)
                if (!divert.isFunctionCall && !(divert.parent is DivertTarget))
                    Error("Functions may not contain diverts, but saw '" + divert + "'", divert);

            var allChoices = _rootWeave.FindAll<Choice>();
            foreach (var choice in allChoices)
                Error("Functions may not contain choices, but saw '" + choice + "'", choice);
        }
        ...
        private void WarningInTermination(Object terminatingObject) {
            ...
            if (terminatingObject.parent == _rootWeave && _firstChildFlow)
                message = message + " Note that if you intend to enter '" + _firstChildFlow.identifier +
