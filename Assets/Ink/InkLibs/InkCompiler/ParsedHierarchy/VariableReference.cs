    ...
    public class VariableReference : Expression {
        ...
        public Identifier identifier {
            get {
                ...
                if (pathIdentifiers == null || pathIdentifiers.Count == 0) return null;

                if (_singleIdentifier == null) {
                    var name = string.Join(".", path.ToArray());
                    var firstDebugMetadata = pathIdentifiers.First().debugMetadata;
                    var debugMetadata =
                        pathIdentifiers.Aggregate(firstDebugMetadata, (acc, id) => acc.Merge(id.debugMetadata));
        ...
        public Runtime.VariableReference runtimeVarRef { get; private set; }
        ...
        public override void GenerateIntoContainer(Container container) {
            ...
            runtimeVarRef = new Runtime.VariableReference(name);
            ...
            if (path.Count == 1 || path.Count == 2) {
                ...
                if (listItem) isListItemReference = true;
            }
        ...
        public override void ResolveReferences(Story context) {
            ...
            if (isConstantReference || isListItemReference) return;
            ...
            if (targetForCount) {
                ...
                if (runtimeVarRef == null) return;

                runtimeVarRef.pathForCount = targetForCount.runtimePath;
                runtimeVarRef.name = null;
            ...
            if (!context.ResolveVariableWithName(name, this).found) Error("Unresolved variable: " + ToString(), this);
        }
