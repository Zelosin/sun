...
namespace Ink.Parsed {
    public class Divert : Object {
        public Divert(Path target, List<Expression> arguments = null) {
            ...
            if (arguments != null) AddContent(arguments.Cast<Object>().ToList());
        }
        ...
        public bool isEnd => target != null && target.dotSeparatedComponents == "END";

        public bool isDone => target != null && target.dotSeparatedComponents == "DONE";

        public override Runtime.Object GenerateRuntimeObject() {
            // End = end flow immediately
            // Done = return from thread or instruct the flow that it's safe to exit
            if (isEnd) return ControlCommand.End();
            if (isDone) return ControlCommand.Done();
            ...
            var requiresArgCodeGen = arguments != null && arguments.Count > 0;
            if (requiresArgCodeGen || isFunctionCall || isTunnel || isThread) {
                ...
                if (requiresArgCodeGen) {
                    ...
                    if (!isFunctionCall) container.AddContent(ControlCommand.EvalStart());
                    ...
                    for (var i = 0; i < arguments.Count; ++i) {
                        ...
                        if (argExpected != null && argExpected.isByReference) {
                            ...
                            if (targetForCount != null) {
                                Error("can't pass a read count by reference. '" + targetPath.dotSeparatedComponents +
                                      "' is a knot/stitch/label, but '" + target.dotSeparatedComponents +
                    ...
                    if (!isFunctionCall) container.AddContent(ControlCommand.EvalEnd());
                }
                ...
                if (variableTargetName != null) {
                    ...
                    if (resolveResult.found) {
                        ...
                        if (resolveResult.isArgument) {
                            var argument = resolveResult.ownerFlow.arguments
                                .Where(a => a.identifier.name == variableTargetName).First();
                            if (!argument.isDivertTarget)
                                Error(
                                    "Since '" + argument.identifier + "' is used as a variable divert target (on " +
                                    debugMetadata + "), it should be marked as: -> " + argument.identifier,
        ...
        public override void ResolveReferences(Story context) {
            if (isEmpty || isEnd || isDone) return;

            if (targetContent) runtimeDivert.targetPath = targetContent.runtimePath;
            ...
            if (targetFlow) {
                if (!targetFlow.isFunction && isFunctionCall)
                    base.Error(targetFlow.identifier +
            ...
            var targetWasFound = targetContent != null;
            var isBuiltIn = false;
            ...
            // Variable target?
            if (runtimeDivert.variableDivertName != null) return;
        ...
        private void CheckArgumentValidity() {
            ...
            // No error, crikey!
            if (numArgs == 0 && (targetFlow == null || !targetFlow.hasParameters)) return;
            ...
            for (var i = 0; i < paramCount; ++i) {
                ...
                if (flowArg.isDivertTarget) {
                    ...
                    if (!(divArgExpr is DivertTarget) && varRef == null) {
                        Error(
                            "Target '" + targetFlow.identifier +
                            "' expects a divert target for the parameter named -> " + flowArg.identifier + " but saw " +
                            divArgExpr, divArgExpr);
                    }
                    ...
                    // i.e. read count instead of divert target
                        ...
                        if (targetForCount != null)
                            Error("Passing read count of '" + knotCountPath.dotSeparatedComponents +
        ...
        private void CheckExternalArgumentValidity(Story context) {
            var externalName = target.firstComponent;
            ...
            var externalArgCount = external.argumentNames.Count;
            var ownArgCount = 0;
            if (arguments != null) ownArgCount = arguments.Count;
