    ...
    public class DivertTarget : Expression {
        ...
        public override void ResolveReferences(Story context) {
            ...
            while (usageContext && usageContext is Expression) {
                ...
                if (usageParent is BinaryExpression) {
                    ...
                    if (binaryExprParent.opName != "==" && binaryExprParent.opName != "!=") {
                        ...
                        if (!(binaryExprParent.leftExpression is DivertTarget ||
                              binaryExprParent.leftExpression is VariableReference)) badUsage = true;
                        if (!(binaryExprParent.rightExpression is DivertTarget ||
                              binaryExprParent.rightExpression is VariableReference)) badUsage = true;
                    }
                    ...
                    if (!funcCall.isTurnsSince && !funcCall.isReadCount) badUsage = true;
                    foundUsage = true;
                }
                ...
                if (badUsage)
                    Error(
                        ...
                        "' as a function: likeThis(), or check the read count: likeThis, with no arrows?", this);
            ...
            if (_runtimeDivert.hasVariableTarget)
                Error("Since '" + divert.target.dotSeparatedComponents +
                    ...
                    foreach (var arg in targetFlow.arguments)
                        if (arg.isByReference)
                            Error(
                                ...
                                targetFlow.identifier + "' has 'ref " + arg.identifier + "').");
            }
        }
