...
namespace Ink.Parsed {
    public class ConditionalSingleBranch : Object {
        ...
        private readonly Weave _innerWeave;
        ...
        public Expression ownExpression {
            ...
            set {
                ...
                if (_ownExpression) AddContent(_ownExpression);
            }
        }
        ...
        public override Runtime.Object GenerateRuntimeObject() {
                ...
                foreach (var c in _innerWeave.content) {
                    ...
                    if (text)
                        ...
                        if (text.text.StartsWith("else:"))
                            Warning("Saw the text 'else:' which is being treated as content. Did you mean '- else:'?",
                                text);
                }
            ...
            var duplicatesStackValue = matchingEquality && !isElse;
            ...
            if (!isTrueBranch && !isElse) {
                var needsEval = ownExpression != null;
                if (needsEval)
            ...
            if (!isInline) _contentContainer.InsertContent(new StringValue("\n"), 0);
        ...
        private Container GenerateRuntimeForContent() {
            ...
            if (_innerWeave == null) return new Container();
