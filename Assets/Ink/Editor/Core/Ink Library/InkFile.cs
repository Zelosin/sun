...
namespace Ink.UnityIntegration {
    ...
    public class InkFile {
        public bool compileAutomatically;
        ...
        public List<string> unhandledCompileErrors = new();

        // Fatal errors caused by errors in the user's ink script.
        public List<InkCompilerLog> errors = new();

        public List<InkCompilerLog> warnings = new();

        public List<InkCompilerLog> todos = new();
        ...
        public List<DefaultAsset> includes = new();
        ...
        public string jsonPath {
            get {
                ...
                var jsonFolder = jsonAssetDirectory;
                ...
                var jsonPath = AssetDatabase.GetAssetPath(jsonFolder);
                var strJsonAssetPath =
                    InkEditorUtils.CombinePaths(jsonPath, Path.GetFileNameWithoutExtension(_filePath)) + ".json";
        ...
        public DateTime lastCompileDate {
            get {
                if (isMaster) {
                    var fullJSONFilePath =
                        InkEditorUtils.UnityRelativeToAbsolutePath(AssetDatabase.GetAssetPath(jsonAsset));
        ...
        public bool isParent => includes.Count > 0;
        ...
        public bool isMaster => masterInkAssets == null || masterInkAssets.Count == 0;

        // The InkFiles of the includes of this file
        public List<InkFile> includesInkFiles {
            get {
                var _includesInkFiles = new List<InkFile>();
        ...
        public List<InkFile> inkFilesInIncludeHierarchy {
            get {
                var _inkFilesInIncludeHierarchy = new List<InkFile>();
...
//		public string content;
        ...
        public void FindIncludedFiles(bool addMissing = false) {
            ...
            foreach (var includePath in includePaths) {
                var localIncludePath = InkEditorUtils.CombinePaths(Path.GetDirectoryName(filePath), includePath);
                ...
                var includedInkFileAsset = AssetDatabase.LoadAssetAtPath<DefaultAsset>(localIncludePath);
                if (includedInkFileAsset == null) {
                    Debug.LogError(
                        filePath + " expected child .ink asset at '" + localIncludePath + "' but file was not found.",
                        inkAsset);
                }
                else {
                    var includedInkFile = InkLibrary.GetInkFileWithFile(includedInkFileAsset, addMissing);
                    if (includedInkFile == null)
                        Debug.LogError(
                            filePath + " expected child InkFile from .ink asset at '" + localIncludePath +
                            "' but file was not found.", inkAsset);
                    else if (includedInkFile.includes.Contains(inkAsset))
                        Debug.LogError(
                            "Circular INCLUDE reference between '" + filePath + "' and '" + includedInkFile.filePath +
                            "'.", inkAsset);
                    else
        ...
        public static class InkIncludeParser {
            ...
            private static IEnumerable<string> FindIncludes(string str) {
                var matches = includeRegex.Matches(str);
