...
namespace Ink.UnityIntegration {
    internal class CreateInkAssetAction : EndNameEditAction {
        public override void Action(int instanceId, string pathName, string resourceFile) {
            ...
            var asset = CreateScriptAsset(pathName, text);
    ...
    public static class InkEditorUtils {
        ...
        public static bool disallowedAutoRefresh {
            ...
            set => EditorPrefs.SetBool("InkLibraryDisallowedAutoRefresh", value);
        }
        ...
        private static void EnsureFirstLaunchHandled() {
            var lastCompileTime = LoadAndSaveLastCompileTime();
            isFirstCompile = EditorApplication.timeSinceStartup < lastCompileTime;
            if (isFirstCompile)
                OnOpenUnityEditor();
        }

        private static float LoadAndSaveLastCompileTime() {
        ...
        private static void OnOpenUnityEditor() {
        ...
        public static void RecompileAll() {
            ...
            var logString = filesToRecompile.Any()
                ? "Recompile All will compile " + string.Join(", ",
                    filesToRecompile.Select(x => Path.GetFileName(x.filePath)).ToArray())
        ...
        public static void RecompileAllImmediately() {
            var filesToRecompile = InkLibrary.FilesCompiledByRecompileAll().ToArray();
            var logString = filesToRecompile.Any()
                ? "Recompile All Immediately will compile " + string.Join(", ",
                    filesToRecompile.Select(x => Path.GetFileName(x.filePath)).ToArray())
                : "No valid ink found. Note that only files with 'Compile Automatic' checked are compiled if not set to compile all files automatically in InkSettings file.";
            Debug.Log(logString);
        ...
        public static void CreateNewInkFile() {
            var fileName = "New Ink.ink";
            var filePath = AssetDatabase.GenerateUniqueAssetPath(Path.Combine(GetSelectedPathOrFallback(), fileName));
        ...
        public static void CreateNewInkFile(string filePath, string templateFileLocation) {
            ProjectWindowUtil.StartNameEditingIfProjectWindowExists(0,
                ScriptableObject.CreateInstance<CreateInkAssetAction>(), filePath, InkBrowserIcons.inkFileIcon,
        ...
        public static void OnPostprocessBuild(BuildTarget target, string pathToBuiltProject) {
            if (!Debug.isDebugBuild) {
                ...
                Debug.Log("<color=" + color +
                          ">Thanks for using ink, and best of luck with your release!\nIf you're doing well, please help fund the project via Patreon https://www.patreon.com/inkle</color>");
            }
        }

        public static TextAsset CreateStoryStateTextFile(string jsonStoryState, string defaultPath = "Assets/Ink",
            string defaultName = "storyState") {
            var name = AssetDatabase.GenerateUniqueAssetPath(Path.Combine(defaultPath, defaultName + ".json"))
                .Substring(defaultPath.Length + 1);
            var fullPathName = EditorUtility.SaveFilePanel("Save Story State", defaultPath, name, "json");
            if (fullPathName == "")
                return null;
            using (var outfile = new StreamWriter(fullPathName)) {
                outfile.Write(jsonStoryState);
            }

            var relativePath = AbsoluteToUnityRelativePath(fullPathName);
        ...
        public static bool IsInkFile(string path) {
            ...
            if (extension == inkFileExtension) return true;
