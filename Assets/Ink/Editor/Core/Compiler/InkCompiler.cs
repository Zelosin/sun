...
namespace Ink.UnityIntegration {
	...
	public class InkCompiler : ScriptableObject {
        ...
        public static bool compiling {
            get { return instance.compilationStack.Count > 0; }
        }
        ...
        public List<CompilationStackItem> compilationStack = new List<CompilationStackItem>();
        ...
        static void UpdateProgressBar() {
            ...
            string message = "Compiling .Ink File " + (instance.compilationStack.Count - numCompiling) + " of " +
                             instance.compilationStack.Count + ".";
            if (playModeBlocked) message += " Will enter play mode when complete.";
            if (buildBlocked || playModeBlocked || EditorApplication.isPlaying)
        ...
        static void BlockPlayMode() {
            ...
            Debug.LogWarning("Delayed entering play mode because Ink is still compiling (" + percentage +
                             "). Will enter play mode on completion.");
            playModeBlocked = true;
        }

        static void EnteredPlayModeWhenCompiling() {
            Debug.LogError(
                "Entered Play Mode while Ink was still compiling! Your story will not be up to date. Recommend exiting and re-entering play mode.\nWe normally delay entering play mode when compiling, so you've found an edge case!");
        }
        ...
        /// <summary>
        ///     Starts a System.Process that compiles a master ink file, creating a playable JSON file that can be parsed by the
        ///     Ink.Story class
        /// </summary>
        ...
        private static void CompileInkInternal(InkFile inkFile, bool immediate) {
            ...
            if (!inkFile.isMaster)
                Debug.LogWarning(
                    "Compiling InkFile which is an include. Any file created is likely to be invalid. Did you mean to call CompileInk on inkFile.master?");
            ...
            string inputPath =
                InkEditorUtils.CombinePaths(inkFile.absoluteFolderPath, Path.GetFileName(inkFile.filePath));
        ...
        private static void CompleteCompilingFile(CompilationStackItem item) {
            ...
            if (item.timeTaken > InkSettings.instance.compileTimeout * 0.6f)
                Debug.LogWarning("Compilation for " + Path.GetFileName(item.inkFile.filePath) +
                                 " took over 60% of the time required to timeout the compiler. Consider increasing the compile timeout on the InkSettings file.");
        }
        ...
        private static void SetOutputLog(CompilationStackItem pendingFile) {
            ...
            foreach (var output in pendingFile.logOutput) {
                if (output.type == ErrorType.Error) {
                    ...
                    Debug.LogError(
                        "Ink " + output.type + ": " + output.content + " (at " + output.fileName + ":" +
                        output.lineNumber + ")", pendingFile.inkFile.inkAsset);
                }
                    ...
                    if (InkSettings.instance.printInkLogsInConsoleOnCompile)
                        Debug.Log(
                            "Ink Log: " + output.content + " (at " + output.fileName + " " + output.lineNumber + ")",
                            pendingFile.inkFile.inkAsset);
                }
            }
        }
        ...
        public static List<InkFile> GetUniqueMasterInkFilesToCompile(List<string> importedInkAssets) {
            ...
            foreach (var importedAssetPath in importedInkAssets) {
                foreach (var masterInkFile in GetMasterFilesIncludingInkAssetPath(importedAssetPath)) {
                    if (!masterInkFiles.Contains(masterInkFile) && (InkSettings.instance.compileAutomatically ||
                                                                    masterInkFile.compileAutomatically)) {
        ...
        public static void AddToCompilationStack(CompilationStackItem compilationStackItem) {
        ...
        public static int NumFilesInCompilingStackInState(CompilationStackItem.State state) {
            int count = 0;
            foreach (var x in instance.compilationStack) {
                if (x.state == state)
                    count++;
        ...
        public static List<CompilationStackItem> FilesInCompilingStackInState(CompilationStackItem.State state) {
            List<CompilationStackItem> items = new List<CompilationStackItem>();
            foreach (var x in instance.compilationStack) {
                if (x.state == state)
                    items.Add(x);
            }
        ...
        public static CompilationStackItem GetCompilationStackItem(InkFile inkFile) {
            foreach (var x in instance.compilationStack) {
