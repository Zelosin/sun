...
//		public class WindowState {
        ...
        static TextAsset storyJSONTextAsset {
            get { return _storyJSONTextAsset; }
            set {
                if (_storyJSONTextAsset == value) return;
                _storyJSONTextAsset = value;
                if (_storyJSONTextAsset != null) {
                    InkPlayerWindowState.Instance.lastStoryJSONAssetPath =
                        AssetDatabase.GetAssetPath(storyJSONTextAsset);
                    string fullJSONFilePath =
                        InkEditorUtils.UnityRelativeToAbsolutePath(InkPlayerWindowState.Instance
                            .lastStoryJSONAssetPath);
        ...
        public static TextAsset storyStateTextAsset {
            get { return _storyStateTextAsset; }
            set {
                ...
                if (_storyStateTextAsset != null)
                    storyStateValid =
                        InkEditorUtils.CheckStoryStateIsValid(storyJSONTextAsset.text, storyStateTextAsset.text);
            }
        }
        ...
        void OnGUI() {
            ...
            if (doingAutoscroll) {
                var newY = Mathf.SmoothDamp(InkPlayerWindowState.Instance.storyPanelState.scrollPosition.y,
                    autoscrollTarget, ref autoscrollVelocity, autoscrollSmoothTime, Mathf.Infinity, deltaTime);
                InkPlayerWindowState.Instance.storyPanelState.scrollPosition =
                    new Vector2(InkPlayerWindowState.Instance.storyPanelState.scrollPosition.x, newY);
            ...
            if (story != null && story.state != null) {
                ...
                EditorGUILayout.LabelField(
                    "You can tether your story as it runs in Play Mode to this window, allowing you to view and edit variables and divert in real time!",
                    EditorStyles.wordWrappedLabel);
                GUILayout.Space(20);
                if (GUILayout.Button("View Documentation")) {
                    Application.OpenURL(
                        "https://github.com/inkle/ink-unity-integration/blob/master/Documentation/InkPlayerWindow.md");
                }
        ...
        private void OnBecameVisible() {
            if (doingAutoscroll) {
                InkPlayerWindowState.Instance.storyPanelState.scrollPosition =
                    new Vector2(InkPlayerWindowState.Instance.storyPanelState.scrollPosition.x, autoscrollTarget);
                doingAutoscroll = false;
            }
        }
        ...
        static void OnCompleteEvaluateFunction(string functionName, object[] arguments, string textOutput,
            object result) {
            StringBuilder sb = new StringBuilder(functionName);
            if (arguments != null && arguments.Length > 0) {
                sb.Append(" with args: ");
        ...
        public static void Attach(Story story) {
            Attach(story, InkPlayerParams.ForAttachedStories);
        }
        ...
        public static void LoadAndPlay(TextAsset storyJSONTextAsset) {
            GetWindow();
            if (story != null) {
                if (EditorUtility.DisplayDialog("Story in progress",
                        "The Ink Player Window is already playing a story. Would you like to stop it and load the new story?",
                        "Stop and load", "Cancel")) {
                    Stop();
        ...
        static bool TryPrepareInternal(TextAsset newStoryJSONTextAsset) {
            ...
            if (storyJSONTextAsset == null ||
                !InkEditorUtils.CheckStoryIsValid(storyJSONTextAsset.text, out playStoryException))
                return false;
        ...
        static void OnUnsetStory() {
            ...
            foreach (var observedVariableName in InkPlayerWindowState.Instance.observedVariablesPanelState
                         .observedVariableNames) {
            ...
            foreach (var input in InkPlayerWindowState.Instance.functionPanelState.functionParams.inputs) {
                if (input.type == FunctionPanelState.FunctionParams.FunctionInput.FunctionInputType.InkVariable) {
                    input.RefreshInkVariableValue(story);
                         ...
                         FunctionPanelState.FunctionParams.FunctionInput.FunctionInputType.InkListVariable) {
                    input.RefreshInkListVariableValue(story);
                }
            }

            // Reobserve variables
            var variablesToObserve =
                new List<string>(InkPlayerWindowState.Instance.observedVariablesPanelState.observedVariableNames);
            InkPlayerWindowState.Instance.observedVariablesPanelState.observedVariables.Clear();
        ...
        static void AddToStateHistory() {
            InkPlayerHistoryItem historyItem =
                new InkPlayerHistoryItem(story.state.ToJson(), new List<InkHistoryContentItem>(storyHistory));
        ...
        static void SaveStoryState(string storyStateJSON) {
            ...
            if (storyJSONTextAsset != null) {
                dirPath = Path.GetDirectoryName(AssetDatabase.GetAssetPath(storyJSONTextAsset));
                storyName = storyJSONTextAsset.name;
            }

            storyStateTextAsset =
                InkEditorUtils.CreateStoryStateTextFile(storyStateJSON, dirPath, storyName + "_SaveState");
        }
        ...
        void DisplayHeader() {
            if (attached) {
                ...
                var headerTitle = new StringBuilder("Attached");
                if (attachedWhileInPlayMode != EditorApplication.isPlaying) {
                    if (attachedWhileInPlayMode) headerTitle.Append(" (Ex-play-mode story)");
                }

                GUILayout.Label(new GUIContent(headerTitle.ToString(),
                    "This story reference has been attached from elsewhere"));
                if (GUILayout.Button(new GUIContent("Detach", "Detach from the loaded external story"),
                ...
                storyJSONTextAsset =
                    EditorGUILayout.ObjectField("Story JSON", storyJSONTextAsset, typeof(TextAsset),
                        false) as TextAsset;
                ...
                if (storyJSONTextAsset != null && storyJSON != null) {
                    string fullJSONFilePath =
                        InkEditorUtils.UnityRelativeToAbsolutePath(AssetDatabase.GetAssetPath(storyJSONTextAsset));
                    ...
                    if (currentStoryJSONLastEditDateTime != updatedStoryJSONLastEditDateTime) {
                        ...
                        EditorGUILayout.HelpBox("Story JSON file has changed. Reload or restart to play updated story.",
                            MessageType.Warning);
                        EditorGUILayout.BeginVertical();
        ...
        void DisplayStoryControlGroup() {
            ...
            if (story != null) {
                ...
                if (GUILayout.Button(new GUIContent("Undo", "Undo the last continue or choice"),
                ...
                if (GUILayout.Button(new GUIContent("Redo", "Redo the last continue or choice"),
                        EditorStyles.toolbarButton, GUILayout.Width(36))) {
                    Redo();
                }
            ...
            EditorGUI.BeginDisabledGroup(playerParams.disablePlayControls);
            EditorGUI.BeginChangeCheck();
            var newContinueAutomatically =
                GUILayout.Toggle(playerOptions.continueAutomatically && !playerParams.disablePlayControls,
                    new GUIContent("Auto-Continue", "Continues content automatically"), EditorStyles.toolbarButton);
            if (EditorGUI.EndChangeCheck()) {
            ...
            EditorGUI.BeginChangeCheck();
            var newChooseAutomatically =
                GUILayout.Toggle(playerOptions.chooseAutomatically && !playerParams.disablePlayControls,
                    new GUIContent("Auto-Choice", "Makes choices automatically"), EditorStyles.toolbarButton);
            if (EditorGUI.EndChangeCheck()) {
                if (!playerParams.disablePlayControls) playerOptions.chooseAutomatically = newChooseAutomatically;
            }

            EditorGUI.EndDisabledGroup();
            playerOptions.continueAutomaticallyTimeInterval = playerOptions.chooseAutomaticallyTimeInterval =
                GUILayout.HorizontalSlider(playerOptions.continueAutomaticallyTimeInterval, 1f, 0f,
                    GUILayout.Width(80));
        ...
        void HandleDragAndDrop() {
            ...
            HandlePanelResize(InkPlayerWindowState.Instance.storyPanelState, StoryPanelState.minScrollRectHeight,
                StoryPanelState.maxScrollRectHeight);
            HandlePanelResize(InkPlayerWindowState.Instance.namedContentPanelState, StoryPanelState.minScrollRectHeight,
                StoryPanelState.maxScrollRectHeight);
        }
...
//		}
            ...
            public class FunctionParams {
                ...
                public class FunctionInput {
                    ...
                    public void RefreshInkVariableValue(Story story) {
                        if (!string.IsNullOrWhiteSpace(inkVariablePath))
                            inkVariableValue = story.variablesState[inkVariablePath];
        ...
        public class DisplayOptions {
            ...
            public VisibilityOptions visibilityOptions =
                VisibilityOptions.Warnings | VisibilityOptions.Errors | VisibilityOptions.Content;
        ...
        /// <summary>
        ///     Draws a property field for a story using GUILayout, allowing you to attach stories to the player window for
        ///     debugging.
        /// </summary>
            ...
            if (EditorApplication.isPlaying) {
                if (story != null) {
                    if (isOpen) {
                        ...
                        if (attached && InkPlayerWindow.story == story) {
        ...
        public struct InkPlayerParams {
            ...
            public static InkPlayerParams Standard {
                get { return new InkPlayerParams(); }
            }
        ...
        void DisplayStoryHeader() {
            ...
            InkPlayerWindowState.Instance.storyPanelState.showing =
                EditorGUILayout.Foldout(InkPlayerWindowState.Instance.storyPanelState.showing, "Content", true);

            if (GUILayout.Button(new GUIContent("Clear", "Clears the output"), EditorStyles.toolbarButton,
            ...
            GUILayout.Space(6);
            if (GUILayout.Button(new GUIContent("Copy", "Copy the output to clipboard"), EditorStyles.toolbarButton,
                    GUILayout.ExpandWidth(false))) {
            ...
            void DrawVisibilityOptions() {
                Enum newVisibilityOptions = EditorGUILayout.EnumFlagsField(GUIContent.none,
                    InkPlayerWindowState.Instance.storyPanelState.displayOptions.visibilityOptions,
                    EditorStyles.toolbarDropDown, GUILayout.Width(80));
                InkPlayerWindowState.Instance.storyPanelState.displayOptions.visibilityOptions =
                    (DisplayOptions.VisibilityOptions)(int)Convert.ChangeType(newVisibilityOptions,
                        typeof(DisplayOptions.VisibilityOptions));
        ...
        static bool ShouldShowContentWithSearchString(string contentString, string searchString) {
            if (StringContains(contentString, InkPlayerWindowState.Instance.storyPanelState.searchString,
                ...
                case InkHistoryContentItem.ContentType.PresentedContent: {
                    if ((visibilityOpts & DisplayOptions.VisibilityOptions.Content) != 0) {
                        if (content.content.Length == 0 &&
                            (visibilityOpts & DisplayOptions.VisibilityOptions.EmptyEntries) == 0) return false;
                        else return true;
                    }
                    else return false;
                }
        ...
        static void RefreshVisibleHistory() {
            ...
            for (int i = 0; i < count; i++) {
                ...
                if (doingSearch && !ShouldShowContentWithSearchString(content.content,
                        InkPlayerWindowState.Instance.storyPanelState.searchString)) continue;
        ...
        void DisplayStoryBody() {
            ...
            for (int i = 0; i < validHistory.Count; i++) {
                ...
                if (showTags) {
                    var tagsHeight =
                        EditorStyles.wordWrappedLabel.CalcHeight(new GUIContent(GetTagsString(content.tags)),
                            tagsWidth);
            ...
            var viewportRect = new Rect(0, lastRect.yMax, position.width,
                InkPlayerWindowState.Instance.storyPanelState.height);
            ...
            var newScrollPos = GUI.BeginScrollView(viewportRect,
                InkPlayerWindowState.Instance.storyPanelState.scrollPosition, containerRect, false, true);
            ...
            for (int i = 0; i < validHistory.Count; i++) {
                ...
                if (panelTop <= endY && panelBottom >= y) {
                    ...
                    var lineRect = new Rect(lineContainerRect.x + contentMarginXY,
                        lineContainerRect.y + contentMarginXY, lineContainerRect.width - contentMarginXY * 2,
                        lineContainerRect.height - contentMarginXY * 2);
                    ...
                    if (Event.current.type == EventType.MouseDown &&
                        lineContainerRect.Contains(Event.current.mousePosition)) {
                        if (Event.current.button == 0) {
                            ...
                            Debug.Log(viewportRect + " " +
                                      Mathf.Abs(
                                          targetY - InkPlayerWindowState.Instance.storyPanelState.scrollPosition.y));
                            if (Mathf.Abs(targetY - InkPlayerWindowState.Instance.storyPanelState.scrollPosition.y) >
                                viewportRect.height * 0.225f) {
            ...
            if (Event.current.type == EventType.Layout) {
                if (markedForScrollToBottom != AutoScrollMode.NONE) {
                    ...
                    if (markedForScrollToBottom == AutoScrollMode.Smooth) {
                        ...
                        doingAutoscroll = false;
                        InkPlayerWindowState.Instance.storyPanelState.scrollPosition =
                            new Vector2(InkPlayerWindowState.Instance.storyPanelState.scrollPosition.x, targetPosition);
                    }
        ...
        static ColoredBackgroundGUIStyle historyItemBGStyleDark {
            get {
                if (_historyItemBGStyleDark == null)
                    _historyItemBGStyleDark = new ColoredBackgroundGUIStyle(
                        new Color(0.8470589f, 0.8470589f, 0.8470589f, 1), new Color(0.21f, 0.21f, 0.21f, 1f),
                        new Color(0.92f, 0.92f, 0.92f, 1), new Color(0.3f, 0.3f, 0.3f, 1f));
        ...
        static ColoredBackgroundGUIStyle historyItemBGStyleLight {
            get {
                if (_historyItemBGStyleLight == null)
                    _historyItemBGStyleLight = new ColoredBackgroundGUIStyle(
                        new Color(0.8745099f, 0.8745099f, 0.8745099f, 1f), new Color(0.23f, 0.23f, 0.23f, 1f),
                        new Color(0.92f, 0.92f, 0.92f, 1), new Color(0.3f, 0.3f, 0.3f, 1f));
        ...
        static ColoredBackgroundGUIStyle historyItemBGStyleSelected {
            get {
                if (_historyItemBGStyleSelected == null)
                    _historyItemBGStyleSelected = new ColoredBackgroundGUIStyle(
                        new Color(0.3920879f, 0.6161963f, 0.9339623f, 1f),
                        new Color(0.243137255f, 0.37254902f, 0.588235294f, 1f));
        ...
        static ColoredBackgroundGUIStyle dividerLineStyle {
            get {
                if (_dividerLineStyle == null)
                    _dividerLineStyle = new ColoredBackgroundGUIStyle(new Color(0.85f, 0.85f, 0.85f, 1f),
        ...
        void DisplayLineIcon(Rect rect, InkHistoryContentItem content) {
            var visibilityOptions = InkPlayerWindowState.Instance.storyPanelState.displayOptions.visibilityOptions;
            if (content.contentType == InkHistoryContentItem.ContentType.ChooseChoice &&
                (visibilityOptions & DisplayOptions.VisibilityOptions.SelectedChoice) != 0) {
                var icon = new GUIContent("*", "Selected Choice");
                EditorGUI.LabelField(rect, icon);
            }
            else if (content.contentType == InkHistoryContentItem.ContentType.PresentedChoice &&
                     (visibilityOptions & DisplayOptions.VisibilityOptions.PresentedChoices) != 0) {
                var icon = new GUIContent("*?", "Presented Choice");
                EditorGUI.LabelField(rect, icon);
            }
            else if (content.contentType == InkHistoryContentItem.ContentType.EvaluateFunction &&
                     (visibilityOptions & DisplayOptions.VisibilityOptions.Function) != 0) {
                var icon = new GUIContent("f(x)", "Took Function");
                EditorGUI.LabelField(rect, icon);
            }
            else if (content.contentType == InkHistoryContentItem.ContentType.ChoosePathString &&
                     (visibilityOptions & DisplayOptions.VisibilityOptions.ChoosePathString) != 0) {
                var icon = new GUIContent("->", "Took Path");
                EditorGUI.LabelField(rect, icon);
            }
            else if (content.contentType == InkHistoryContentItem.ContentType.DebugNote &&
                     (visibilityOptions & DisplayOptions.VisibilityOptions.DebugNotes) != 0) {
                var icon = new GUIContent("//", "Debug Note");
                EditorGUI.LabelField(rect, icon);
            }
            else if (content.contentType == InkHistoryContentItem.ContentType.Warning &&
                     (visibilityOptions & DisplayOptions.VisibilityOptions.Warnings) != 0) {
                var icon = EditorGUIUtility.IconContent("console.warnicon.sml");
                icon.tooltip = "Warning";
                EditorGUI.LabelField(rect, icon);
            }
            else if (content.contentType == InkHistoryContentItem.ContentType.Error &&
                     (visibilityOptions & DisplayOptions.VisibilityOptions.Errors) != 0) {
        ...
        void DrawChoices() {
            EditorGUILayout.BeginHorizontal(EditorStyles.toolbar);
            InkPlayerWindowState.Instance.choicePanelState.showing =
                EditorGUILayout.Foldout(InkPlayerWindowState.Instance.choicePanelState.showing, "Choices", true);
            EditorGUILayout.EndHorizontal();
        ...
        void DisplayChoices() {
            ...
            if (story.canContinue) {
                ...
                foreach (Choice choice in story.currentChoices) {
                    ...
                    if (GUILayout.Button(new GUIContent(choice.text.Trim(),
                            "Index: " + choice.index.ToString() + "\nSourcePath: " + choice.sourcePath.Trim()))) {
                        MakeChoice(choice);
                    }
        ...
        static void MakeRandomChoice() {
            MakeChoice(story.currentChoices[Random.Range(0, story.currentChoices.Count)]);
        }
        ...
        static void DrawSaveLoad() {
            EditorGUILayout.BeginHorizontal(EditorStyles.toolbar);
            InkPlayerWindowState.Instance.saveLoadPanelState.showing =
                EditorGUILayout.Foldout(InkPlayerWindowState.Instance.saveLoadPanelState.showing, "Story State", true);
            EditorGUILayout.EndHorizontal();
        ...
        static void DrawSaveLoadPanel() {
            ...
            EditorGUI.BeginChangeCheck();
            storyStateTextAsset =
                EditorGUILayout.ObjectField("Load Story State JSON File", storyStateTextAsset, typeof(TextAsset),
                    false) as TextAsset;
        ...
        void DrawNamedContentHeader() {
            EditorGUILayout.BeginHorizontal(EditorStyles.toolbar);
            InkPlayerWindowState.Instance.namedContentPanelState.showing =
                EditorGUILayout.Foldout(InkPlayerWindowState.Instance.namedContentPanelState.showing, "Named Content",
                    true);
        ...
        void DrawNamedContentPanel() {
            ...
            List<string> paths = new List<string>();
            {
                ...
                void AddContent(string currentPath, KeyValuePair<string, Object> contentKVP, int indent = 0) {
                    if (SearchStringMatch(currentPath,
                            InkPlayerWindowState.Instance.namedContentPanelState.searchString)) {
            ...
            var viewportRect = new Rect(0, lastRect.yMax, position.width,
                InkPlayerWindowState.Instance.namedContentPanelState.height);
            if (mainScrollViewActive) viewportRect.width -= GUI.skin.verticalScrollbar.fixedWidth;
            var containerRect = new Rect(0, 0, containerWidth, totalHeight);

            var newScrollPos = GUI.BeginScrollView(viewportRect,
                InkPlayerWindowState.Instance.namedContentPanelState.scrollPosition, containerRect, false, true);
            ...
            var panelBottom = InkPlayerWindowState.Instance.namedContentPanelState.scrollPosition.y +
                              viewportRect.height;
            ...
            for (int i = 0; i < paths.Count; i++) {
                if (panelTop <= rects[i].yMax && panelBottom >= rects[i].yMin) {
                    ...
                    var lineRect = new Rect(lineContainerRect.x + contentMarginX, lineContainerRect.y + contentMarginY,
                        lineContainerRect.width - contentMarginX * 2, lineContainerRect.height - contentMarginY * 2);
        ...
        void DrawNamedContentItem(Rect rect, string currentPath) {
            ...
            EditorGUI.LabelField(new Rect(rect.xMax - 200, rect.y, 32, rect.height),
                new GUIContent(story.state.VisitCountAtPathString(currentPath).ToString(), "Read count"));
            if (GUI.Button(new Rect(rect.xMax - 168, rect.y, 80, rect.height), new GUIContent("Divert"))) {
                ...
                try {
                    ...
                    Debug.LogWarning("Could not divert to " + currentPath +
                                     "! Only Knots and Stitches can be diverted to. Is this a function? Alternatively, the path might lead to an error, which we prevent from occuring in this tool to safeguard the state.\n" +
                                     e.ToString());
                    hadError = true;
                }
                finally {
        ...
        void DrawFunctionsHeader() {
            EditorGUILayout.BeginHorizontal(EditorStyles.toolbar);
            InkPlayerWindowState.Instance.functionPanelState.showing =
                EditorGUILayout.Foldout(InkPlayerWindowState.Instance.functionPanelState.showing, "Functions", true);
            EditorGUILayout.EndHorizontal();
        }
        ...
        void DrawFunctionInput() {
            ...
            EditorGUI.BeginChangeCheck();
            InkPlayerWindowState.Instance.functionPanelState.functionParams.functionName = EditorGUILayout.TextField(
                "Function Name", InkPlayerWindowState.Instance.functionPanelState.functionParams.functionName);
            if (EditorGUI.EndChangeCheck()) {
            ...
            bool functionIsValid =
                InkPlayerWindowState.Instance.functionPanelState.functionParams.functionName != String.Empty &&
                story.HasFunction(InkPlayerWindowState.Instance.functionPanelState.functionParams.functionName);
            EditorGUI.BeginDisabledGroup(!functionIsValid);
            if (GUILayout.Button(new GUIContent("Execute", "Runs the function"))) {
                AddToHistory(InkHistoryContentItem.CreateForDebugNote("Execute function '" +
                                                                      InkPlayerWindowState.Instance.functionPanelState
                                                                          .functionParams.functionName + "'"));
                string outputContent = null;
                object[] allInput =
                    new object[InkPlayerWindowState.Instance.functionPanelState.functionParams.inputs.Count];
                ...
                InkPlayerWindowState.Instance.functionPanelState.functionReturnValue = story.EvaluateFunction(
                    InkPlayerWindowState.Instance.functionPanelState.functionParams.functionName, out outputContent,
                    allInput);
                if (outputContent != null)
                    AddStoryContent(outputContent, null);
                InkPlayerWindowState.Instance.functionPanelState.testedFunctionName = InkPlayerWindowState.Instance
                    .functionPanelState.functionParams.functionName;
            }
        ...
        void DrawFunctionOutput() {
            bool functionIsValid =
                InkPlayerWindowState.Instance.functionPanelState.functionParams.functionName != String.Empty &&
                story.HasFunction(InkPlayerWindowState.Instance.functionPanelState.functionParams.functionName);
            if (functionIsValid && InkPlayerWindowState.Instance.functionPanelState.functionParams.functionName ==
                InkPlayerWindowState.Instance.functionPanelState.testedFunctionName) {
                GUILayout.BeginVertical(GUI.skin.box);
                if (InkPlayerWindowState.Instance.functionPanelState.functionReturnValue == null) {
                    ...
                    EditorGUILayout.TextField("Output (String)",
                        (string)InkPlayerWindowState.Instance.functionPanelState.functionReturnValue);
                }
                else if (InkPlayerWindowState.Instance.functionPanelState.functionReturnValue is float) {
                    EditorGUILayout.FloatField("Output (Float)",
                        (float)InkPlayerWindowState.Instance.functionPanelState.functionReturnValue);
                }
                else if (InkPlayerWindowState.Instance.functionPanelState.functionReturnValue is int) {
                    EditorGUILayout.IntField("Output (Int)",
                        (int)InkPlayerWindowState.Instance.functionPanelState.functionReturnValue);
                }
                else if (InkPlayerWindowState.Instance.functionPanelState.functionReturnValue is InkList) {
                    EditorGUILayoutInkListField(new GUIContent("Output (InkList)"),
                        (InkList)InkPlayerWindowState.Instance.functionPanelState.functionReturnValue);
                }
                else {
                    EditorGUILayout.LabelField("Function returned unexpected type " + InkPlayerWindowState.Instance
                        .functionPanelState.functionReturnValue.GetType().Name + ".");
                }
        ...
        void BuildFunctionInputList() {
            functionInputList =
                new ReorderableList(InkPlayerWindowState.Instance.functionPanelState.functionParams.inputs,
                    typeof(FunctionPanelState.FunctionParams.FunctionInput), true, true, true, true);
            functionInputList.drawHeaderCallback = (Rect rect) => { EditorGUI.LabelField(rect, "Inputs"); };
            functionInputList.elementHeight =
            ...
            functionInputList.drawElementCallback = (Rect rect, int index, bool isActive, bool isFocused) => {
                ...
                input.type =
                    (FunctionPanelState.FunctionParams.FunctionInput.FunctionInputType)EditorGUI.EnumPopup(typeRect,
                        input.type);
                ...
                switch (input.type) {
                    ...
                    case FunctionPanelState.FunctionParams.FunctionInput.FunctionInputType.Bool:
                        ...
                        break;
                    case FunctionPanelState.FunctionParams.FunctionInput.FunctionInputType.InkVariable: {
                        var halfInput = new Rect(inputRect.x, inputRect.y, Mathf.RoundToInt(inputRect.width * 0.5f) - 5,
                            inputRect.height);
                        var halfInput2 = new Rect(inputRect.x + Mathf.RoundToInt(inputRect.width * 0.5f) + 5,
                            inputRect.y, Mathf.RoundToInt(inputRect.width * 0.5f) - 10, inputRect.height);
                        EditorGUI.BeginChangeCheck();
        ...
        void DrawVariablesHeader() {
            EditorGUILayout.BeginHorizontal(EditorStyles.toolbar);
            InkPlayerWindowState.Instance.variablesPanelState.showing =
                EditorGUILayout.Foldout(InkPlayerWindowState.Instance.variablesPanelState.showing, "Variables", true);
        ...
        void DrawVariablesPanel() {
            ...
            InkPlayerWindowState.Instance.variablesPanelState.scrollPosition =
                EditorGUILayout.BeginScrollView(InkPlayerWindowState.Instance.variablesPanelState.scrollPosition);
            ...
            if (variableToChange != null) {
                AddToHistory(InkHistoryContentItem.CreateForDebugNote("Change '" + variableToChange + "' from '" +
                                                                      story.variablesState[variableToChange] +
                                                                      "' to '" + newVariableValue + "'"));
        ...
        void DrawObservableVariable(string variable, ref string variableToChange, ref object newVariableValue) {
            ...
            if (InkPlayerWindowState.Instance.observedVariablesPanelState.observedVariables.ContainsKey(variable)) {
                ...
                if (GUILayout.Button(new GUIContent("->", "Click to observe this variable, tracking changes"),
                        GUILayout.Width(24))) {
        ...
        static ObservedVariable ObserveVariable(string variableName, bool alsoAddToCache) {
            if (InkPlayerWindowState.Instance.observedVariablesPanelState.observedVariables.ContainsKey(variableName))
                return InkPlayerWindowState.Instance.observedVariablesPanelState.observedVariables[variableName];
            ...
            InkPlayerWindowState.Instance.observedVariablesPanelState.observedVariables.Add(variableName,
                observedVariable);
            if (alsoAddToCache) {
                InkPlayerWindowState.Instance.observedVariablesPanelState.observedVariableNames.Add(variableName);
                if (InkPlayerWindowState.Instance.observedVariablesPanelState.observedVariables.Count !=
                    InkPlayerWindowState.Instance.observedVariablesPanelState.observedVariableNames.Count) {
                    Debug.LogError(InkPlayerWindowState.Instance.observedVariablesPanelState.observedVariables.Count +
                                   " " + InkPlayerWindowState.Instance.observedVariablesPanelState.observedVariableNames
                                       .Count);
                    InkPlayerWindowState.Instance.observedVariablesPanelState.observedVariableNames.Clear();
                    InkPlayerWindowState.Instance.observedVariablesPanelState.observedVariables.Clear();
                }
        ...
        static void UnobserveVariable(string variableName, bool alsoRemoveFromCache) {
            ...
            var observedVariable =
                InkPlayerWindowState.Instance.observedVariablesPanelState.observedVariables[variableName];
        ...
        object DrawVariable(GUIContent guiContent, string variableName, object variableValue) {
            ...
            if (variableValue is string) {
            ...
            else if (variableValue is Runtime.Path) {
                var c = new GUIContent(((Runtime.Path)variableValue).ToString() + " (Ink.Runtime.Path)");
                ...
                EditorGUILayout.LabelField(guiContent,
                    new GUIContent("InkPlayerError: Variable is of unexpected type " + variableValue.GetType().Name +
                                   "."));
            }
        ...
        void DrawObservedVariablesHeader() {
            EditorGUILayout.BeginHorizontal(EditorStyles.toolbar);
            InkPlayerWindowState.Instance.observedVariablesPanelState.showing = EditorGUILayout.Foldout(
                InkPlayerWindowState.Instance.observedVariablesPanelState.showing, "Observed Variables", true);
            EditorGUILayout.EndHorizontal();
        }

        void DrawObservedVariablesPanel() {
            List<string> allToRemove = new List<string>();
            foreach (var observedVariable in
                     InkPlayerWindowState.Instance.observedVariablesPanelState.observedVariables) {
        ...
        bool DrawObservedVariable(ObservedVariable observedVariable) {
            GUILayout.BeginHorizontal();
            observedVariable.expanded =
                EditorGUILayout.Foldout(observedVariable.expanded, observedVariable.variable, true);
            ...
            if (observedVariable.expanded) {
                ...
                observedVariable.scrollPosition =
                    EditorGUILayout.BeginScrollView(observedVariable.scrollPosition, GUI.skin.box);

                foreach (var value in observedVariable.values) {
                    DrawVariable(new GUIContent(value.dateTime.ToLongTimeString()), observedVariable.variable,
                        value.state);
                }
        ...
        bool isProfiling {
            get { return _currentStoryProfiler != null; }
            set {
        ...
        Profiler _currentStoryProfiler;
        Profiler _previousStoryProfiler;
        ...
        void DrawProfilerData() {
            ...
            EditorGUILayout.BeginHorizontal(EditorStyles.toolbar);
            InkPlayerWindowState.Instance.profilerPanelState.showing =
                EditorGUILayout.Foldout(InkPlayerWindowState.Instance.profilerPanelState.showing, "Profiler data",
                    true);
            GUILayout.FlexibleSpace();
            ...
            if (InkPlayerWindowState.Instance.profilerPanelState.showing) {
                if (isProfiling) {
                    EditorGUILayout.LabelField("Click 'Stop Profiling' to show profiling results.",
        ...
        static bool SearchStringMatch(string content, string searchString) {
            return string.IsNullOrWhiteSpace(searchString) ||
                   StringContains(content, searchString, StringComparison.OrdinalIgnoreCase);
        }


        static void EditorGUILayoutInkListField(GUIContent guiContent, InkList inkList,
            string expandedVariableKey = null) {
            if (inkList.Any()) {
                var show = expandedVariableKey == null
                    ? true
                    : InkPlayerWindowState.Instance.variablesPanelState.expandedVariables.Contains(expandedVariableKey);
                ...
                if (EditorGUI.EndChangeCheck() && expandedVariableKey != null) {
                    if (show)
                        InkPlayerWindowState.Instance.variablesPanelState.expandedVariables.Add(expandedVariableKey);
                    else
                        InkPlayerWindowState.Instance.variablesPanelState.expandedVariables.Remove(expandedVariableKey);
                }
    ...
    public class UndoHistory<T> where T : class {
        ...
        public bool canUndo {
            get { return undoHistory.Count > 0 && undoHistoryIndex > 0; }
        }

        public bool canRedo {
            get { return undoHistory.Count > 0 && undoHistoryIndex < undoHistory.Count - 1; }
        }
